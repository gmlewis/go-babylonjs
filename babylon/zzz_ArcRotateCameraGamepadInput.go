// Code generated by docs2go. DO NOT EDIT.

package babylon

import (
	"syscall/js"
)

// ArcRotateCameraGamepadInput represents a babylon.js ArcRotateCameraGamepadInput.
// Manage the gamepad inputs to control an arc rotate camera.

//
// See: http://doc.babylonjs.com/how_to/customizing_camera_inputs
type ArcRotateCameraGamepadInput struct{}

// JSObject returns the underlying js.Value.
func (a *ArcRotateCameraGamepadInput) JSObject() js.Value { return a.p }

// ArcRotateCameraGamepadInput returns a ArcRotateCameraGamepadInput JavaScript class.
func (b *Babylon) ArcRotateCameraGamepadInput() *ArcRotateCameraGamepadInput {
	p := b.ctx.Get("ArcRotateCameraGamepadInput")
	return ArcRotateCameraGamepadInputFromJSObject(p)
}

// ArcRotateCameraGamepadInputFromJSObject returns a wrapped ArcRotateCameraGamepadInput JavaScript class.
func ArcRotateCameraGamepadInputFromJSObject(p js.Value) *ArcRotateCameraGamepadInput {
	return &ArcRotateCameraGamepadInput{p: p}
}

// NewArcRotateCameraGamepadInput returns a new ArcRotateCameraGamepadInput object.
//
// https://doc.babylonjs.com/api/classes/babylon.arcrotatecameragamepadinput
func (b *Babylon) NewArcRotateCameraGamepadInput(todo parameters) *ArcRotateCameraGamepadInput {
	p := b.ctx.Get("ArcRotateCameraGamepadInput").New(todo)
	return ArcRotateCameraGamepadInputFromJSObject(p)
}

// TODO: methods
