...
func main() {
	...
	createScene := func() *babylon.Scene {
		...
		scene.SetClearColor(b.NewColor3(0.5, 0.5, 0.5))
		...
		/*******************************Local Axes****************************/
		localAxes := func(size float64) {
			var pilot_local_axisX = b.CreateLines("pilot_local_axisX", []*babylon.Vector3{
				b.Vector3().Zero(), b.NewVector3(size, 0, 0), b.NewVector3(size*0.95, 0.05*size, 0),
				b.NewVector3(size, 0, 0), b.NewVector3(size*0.95, -0.05*size, 0),
			}, scene)
			pilot_local_axisX.SetColor(b.NewColor3(1, 0, 0))

			pilot_local_axisY = b.CreateLines("pilot_local_axisY", []*babylon.Vector3{
				b.Vector3().Zero(), b.NewVector3(0, size, 0), b.NewVector3(-0.05*size, size*0.95, 0),
				b.NewVector3(0, size, 0), b.NewVector3(0.05*size, size*0.95, 0),
			}, scene)
			pilot_local_axisY.SetColor(b.NewColor3(0, 1, 0))

			var pilot_local_axisZ = b.CreateLines("pilot_local_axisZ", []*babylon.Vector3{
				b.Vector3().Zero(), b.NewVector3(0, 0, size), b.NewVector3(0, -0.05*size, size*0.95),
				b.NewVector3(0, 0, size), b.NewVector3(0, 0.05*size, size*0.95),
			}, scene)
			pilot_local_axisZ.SetColor(b.NewColor3(0, 0, 1))

			var localOrigin = b.CreateBox("local_origin", &babylon.BoxOpts{Size: Float64(1)}, scene)
			localOrigin.IsVisible = false

			pilot_local_axisX.parent = localOrigin
			pilot_local_axisY.parent = localOrigin
			pilot_local_axisZ.parent = localOrigin
